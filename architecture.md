...
- **`train` (`source="weights"`)**: additionally probes the adapter weights to compute the recall-miss set, backs up the main tiers, retrains `episodic` / `semantic` / `procedural`, and persists + verifies the weights. The venue also refuses the fold — before any durable write, restoring the pre-fold weights — when a retrained tier's own recall check falls short of its full key set.
...
**Dedup is registry-true.** Two keys collapse iff their registry-true SPO is identical. The collapsed key is **soft-staled** (registry entry retained, simhash retained, excluded from training) rather than deleted. The surviving key inherits the standing of every key merged into it, so a fact that had earned its way into the semantic tier keeps that status through a collapse instead of being served from episodic again. The fold is **additive and lossless** with respect to registered facts: no registered fact is silently erased by a recall miss, and none is silently demoted by a merge.
