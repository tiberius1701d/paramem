...
4. **SOTA enrichment with explicit-binding protocol** (`configs/prompts/sota_enrichment.txt`, `_filter_with_sota`): SOTA returns a JSON object containing `facts`, `entities`, and `new_entity_bindings: {placeholder: real_name}` for any net-new entities it introduced. The pipeline merges these bindings into the reverse mapping before de-anonymization. No transcript token-diff — bindings are declared, not inferred.
...
6. **Plausibility** (`configs/prompts/sota_plausibility.txt` or `_local_plausibility_filter`): residual safety net. Drops self-loops, name-swap pairs, role leaks (`Assistant`/`User`/`Speaker`/...), unresolved placeholders in real-name input, sentinel objects (`Unknown`/`None`/...), and HA-style dot-separated identifiers. Closed-form lexical patterns only — judgment-grade discrimination of SOTA enrichment quality is left to prompt evolution rather than post-hoc deterministic filters.
