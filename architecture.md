...
- **Quadruple encoding** (`"quad"`, production): the adapter is trained directly on the merged-graph triple, `key → JSON{key, subject, predicate, object}` (1 training example per fact, no QA-generator LLM step). Recall template: `"Recall the fact stored under key 'graphN'."` A scalar entity attribute (e.g. a phone number or a hobby) is projected into an ordinary attribute-typed fact at extraction time, so it carries the same speaker attribution and assertion window as any other fact and reaches the keyed set the same way. The quad units come from `assign_keys` over the merged graph's edges and node attributes, partitioned to episodic / procedural by `relation_prep.partition_relations`, then formatted by `format_entry_training` (`paramem/memory/entry.py`). Round-trip-clean; ~½ the per-fact training cost.
