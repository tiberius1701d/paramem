...
- **What to run:** one action per door, named by the intent the door offers: `POST /scheduled-tick` → `AUTO`, `POST /consolidate` → `FULL`, `POST /consolidate/interim` → `INTERIM`, `POST /reconsolidate` → `RECONCILE`. `AUTO` is the scheduled tick's action and the only one the schedule resolves (to `FULL` or `INTERIM`, via `_is_full_cycle_due`); an operator door is a deliberate request and is never re-decided.
- **Whether to run at all:** the **catch-up gate** (a heartbeat wakeup that is not yet due) and the **content gate** (nothing new to consume — no content-bearing interim slot and no attributable pending session → `noop_*`, no GPU work) both belong to the schedule, so both are applied to `AUTO` and to nothing else. There is no bypass flag: the bypass is the arbitrator not asking. Session triage — retiring what can never be attributed — is a side-effect pre-stage that runs on *every* dispatch, so bypassing the content gate never switches orphan retirement off. A manual run does not move the cadence window; the next scheduled tick still has its content gate and noops on its own if the manual run consumed everything.
...
Below the arbitrator the training layer has **no notion of who asked**: `ConsolidationLoop.consolidate(mode=..., keys_from=...)` takes the fold's venue, its key source and its fold inputs, nothing else, and the per-tier floor holds for every caller.
