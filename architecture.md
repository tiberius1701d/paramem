...
- **Scheduled (cooperative):** `_extract_and_start_training` spawns a `BackgroundTrainer` that releases the GPU lock per step so voice turns interleave. Driven by a systemd user timer whose schedule IS `consolidation.refresh_cadence` — the timer never sees the derived full period (`refresh_cadence × max_interim_count`); that derivation is consumed by `_is_full_cycle_due`, which decides per-tick whether a fired timer runs a full fold or an interim cycle. `refresh_cadence` accepts `"HH:MM"` (daily), `"every Nh"`/`"every Nm"` (interval), `"daily"`, or `""`/`"off"` (manual only). Every rendered timer is `OnCalendar` + `Persistent=true`, so a tick missed during suspend/power-off is caught up on resume; cadences that don't divide evenly into a day/hour render at a coarser calendar heartbeat grid and are gated by a durable last-attempt stamp (`paramem/server/schedule_state.py`) rather than by the timer identity alone (see `paramem/server/systemd_timer.py` module docstring).
