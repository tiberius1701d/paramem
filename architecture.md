...
2. **Anonymize** (`configs/prompts/anonymization.txt`): the local model returns ONLY the `{real → placeholder}` mapping — no facts, no transcript (measured across 22 persisted sessions: the model's fact array was a verbatim echo of its input, names swapped, before the prompt stopped asking for facts at all). The SCRIPT builds the anonymized fact array (one entry per `graph.relations`, `subject`/`object` substituted through the mapping, `predicate`/`relation_type`/`confidence` copied verbatim — the predicate is never a substitution target) and the anonymized transcript from that mapping. A fact can therefore never be lost, reworded, or dropped by the anonymizer, and a placeholder can never be glued into a predicate at this stage. Placeholders follow an **open-vocabulary shape contract** (`^[A-Z][A-Za-z]*_\d+$`) — except the speaker's own `speaker{N}` anchor, which is deliberately exempt (already anonymous, never minted, never re-braced) and can appear as a forward-map value without matching that shape.
...
4. **SOTA enrichment with delta protocol** (`configs/prompts/sota_enrichment.txt`): SOTA returns a delta envelope `{add, modify, drop, bindings}` — only the changes against the input fact list, plus `bindings: {placeholder: real_name}` for net-new entities. The pipeline applies the delta, merges bindings before de-anonymization, and reconstructs the updated transcript locally. No transcript token-diff and no fact-echo (order-of-magnitude token reduction vs. the prior "echo every fact" envelope). `add`/`modify` entries are restricted to the fields that actually reach a relation (subject/predicate/object/relation_type/confidence/symmetric) — any other key an LLM invents is stripped before the entry enters the pipeline, so it can never later be mistaken for an unresolved placeholder. A delta that references a token neither in the anonymized facts/transcript SOTA was shown nor in its own `bindings` (orphan), or that binds a token already resolvable against the local map (conflict), is rejected **as a whole unit** — the local-extract facts are kept unchanged rather than partially applied. Rejection is operator-visible: `graph.diagnostics["sota_enrichment_rejected"]` plus an ERROR-level log line naming the offending tokens.
5. **De-anonymize** (`_apply_bindings`): the single deanon exit gate, in three ordered steps. First, a **predicate invariant** run BEFORE substitution drops (never repairs) any fact whose `predicate` field contains a token from the declared placeholder vocabulary (the union of the anonymizer's `reverse` map and SOTA's `bindings`) — the predicate is never a substitution target, so checking it after substitution would silently miss an already-corrupted predicate. Second, **substitution**: deterministic substring replacement of placeholder tokens with their real values, resolved against an observed-scoped map — the local (real) mapping for tokens SOTA was actually shown, SOTA's own `bindings` for tokens it minted, with the local mapping always taking precedence on conflict. Third, a **residual sweep** checks every fact field (subject/predicate/object/relation_type/confidence/symmetric — never a non-fact field an LLM invented) against the same declared vocabulary, plus a placeholder-shaped-token regex as a last-resort, fail-closed backstop for an undeclared orphan the vocabulary check can't see; the regex is never load-bearing for resolution or substitution, only for this final net. Both steps are fail-closed (drop, not repair) and both record counters/lists in `graph.diagnostics` (`predicate_placeholder_dropped` / `predicate_placeholder_dropped_facts` for the predicate invariant, written at the deanon stage — a placeholder glued into a predicate can only arrive in SOTA's *returned* facts now, since the anonymizer stage never produces facts at all — and `residual_dropped_facts` for the residual sweep) — disjoint categories, never double-counted.
...
**No post-hoc leak check between steps 2 and 4** (removed — see SECURITY.md for the residual this leaves and why it was deleted, not weakened). The anonymized payload SOTA sees is built entirely by the script in step 2 from a table it mints and owns (`_build_anonymization_mapping`), substituted through `_substitute_whole_words` — an exact, case-sensitive primitive. There is nothing left for a post-hoc scan to verify at this boundary; what still needs checking is what SOTA sends *back* (step 5), which is a model rewriting content, not a table the script owns.

**Second call site — graph-tier enrichment.** The privacy envelope above (steps 2–6) is not session-tier-only. `ConsolidationLoop._run_graph_enrichment`'s post-merge, cross-session SOTA pass (`paramem.graph.extractor._graph_enrich_with_sota`) runs the SAME anonymize → SOTA → de-anonymize contract, through the SAME shared primitives (`paramem.graph.placeholders`), before any subgraph triple leaves the process.

The cumulative fold graph carries no reliable entity types of its own (registry-derived relations have none), so this pass cannot type entities from node attributes the way session-tier extraction types them from fresh extraction. Instead, before each chunk's SOTA call it runs the SAME local-model anonymizer session-tier extraction uses (`anonymize_with_local_model`) and derives each real name's type from the placeholder the local model minted for it (`placeholder_entity_type` — brace-tolerant, the one site collapsing what were three duplicated inline derivations). The local model is the PRIMARY type source at this tier, so coverage is best-effort and depends on its classification accuracy, exactly as at the session tier (see `SECURITY.md` for why this cannot be independently verified).

Before this pass builds `chunk_entities`, `_run_graph_enrichment` reconciles the local anonymizer's mapping keys onto the chunk's actual node-key text via `canonical()` — a re-cased/separator-varied/diacritic-varied key from the local model (e.g. `"Yang Ming"`) is re-keyed onto the node it names (e.g. `"yang ming"`), and an entry matching no node in the chunk (or an ambiguous multiple) is dropped and counted (`mapping_rekey_dropped`). This reconciliation lives at this ONE call site, not inside the shared `_substitute_whole_words` primitive: `_substitute_whole_words` matches exactly everywhere, including at this tier.

A local mapping that comes back completely empty — or whose every entry this reconciliation drops — for a chunk with real (non-speaker) content is treated as a classification failure, and that chunk's SOTA call is skipped (fail-closed), counted in `privacy_skipped_chunks`. Under the default `{"person"}` cloud-egress scope, the anonymous `speaker{N}` handle is never tokenised at this tier either — the local anonymizer prompt forbids mapping it, so it reaches the payload bare by design (it carries no identifying information).

The response's `relations` and `same_as` pairs are de-anonymized via the same exit gate (`_apply_bindings`) and the free-text primitive (`deanonymize_text`) before `_run_graph_enrichment` ever consumes them — load-bearing for the W1 speaker-pair guard (`is_speaker_id`), which cannot recognise a placeholder token as a speaker id. Accepted consequence: person-level `same_as` coreference (nickname/honorific variants of the same person) is lost under the default scope, since both surfaces collapse to opaque tokens before the model sees them; org/place/thing coreference is unaffected.
...
Every consolidation trigger — the systemd timer and all three operator endpoints — goes through one arbitrator, `_dispatch_consolidation`. It is **non-blocking in every case**: the arbitrator decides, submits the run to an executor, and returns immediately with a `status` and the `action` it resolved to (`interim` or `full`). Progress is observed via `GET /status` (`consolidating`). Nothing runs the fold on the request thread.

The arbitrator owns three decisions the fold itself knows nothing about:

- **Who may run:** a busy server (fold in flight, chat in progress, GPU held, cloud-only) returns `deferred_*`; a migration TRIAL returns 409 `trial_active`.
- **What to run:** `AUTO` (the schedule decides, via `_is_full_cycle_due`), `INTERIM`, or `FULL`. The operator's four doors map onto these: `POST /consolidate` → AUTO, `POST /consolidate/interim` → INTERIM, `POST /reconsolidate` → FULL (schedule and content gate bypassed), `POST /scheduled-tick` → AUTO with the catch-up gate applied.
- **Whether to run at all:** one **content gate**, applied to whichever action was resolved. Nothing new to consume — no content-bearing interim slot and no attributable pending session — means `noop_*`, and no GPU work at all. `POST /reconsolidate` is the single deliberate exception: its input is the stored knowledge, so it always has something to rebuild from.

Below the arbitrator the training layer has **no notion of who asked**: `ConsolidationLoop.consolidate(mode=...)` takes the fold's mode and its fold inputs, nothing else, and the per-tier floor holds for every caller.

The **cooperative training path** (`_extract_and_start_training`) spawns a `BackgroundTrainer` that releases the GPU lock per step so voice turns interleave. Scheduling is driven by a systemd user timer whose schedule IS `consolidation.refresh_cadence` — the timer never sees the derived full period (`refresh_cadence × max_interim_count`, or `refresh_cadence` itself at `max_interim_count: 0`); that derivation is consumed by `_is_full_cycle_due`, which decides per-tick whether a fired timer runs a full fold or an interim cycle. `refresh_cadence` accepts `"HH:MM"` (daily), `"every Nh"`/`"every Nm"` (interval), `"daily"`, or `""`/`"off"` (manual only). Every rendered timer is `OnCalendar` + `Persistent=true`, so a tick missed during suspend/power-off is caught up on resume; cadences that don't divide evenly into a day/hour render at a coarser calendar heartbeat grid and are gated by a durable last-attempt stamp (`paramem/server/schedule_state.py`) rather than by the timer identity alone (see `paramem/server/systemd_timer.py` module docstring).

**`window_stamp` is provenance only.** Main adapter slots are stamped with the full-consolidation window they belong to, and that stamp is written into the slot manifest — but no code compares stamps to decide anything. `_is_full_cycle_due` never reads it. There is consequently no "clear the stamp to force a full cycle" escape hatch (it never worked); the way to force a full rebuild is `POST /reconsolidate`.
