...
4. **Leak guard + repair**: the PII-scoped LLM leak guard is the PII defense (`extraction_verify_anonymization`, default on) — it reconciles the anonymizer's output against the entities extraction named and repairs or blocks. A spaCy NER cross-check is available as an **experimental, off-by-default** second opinion (`extraction_ner_check: false`, installed via the `ner` extra); it is a brittle estimator, not a shipped control, and nothing on the default path depends on it. Placeholders follow an **open-vocabulary shape contract** (`^[A-Z][A-Za-z]*_\d+$`) — except the speaker's own `speaker{N}` anchor, which is deliberately exempt (already anonymous, never minted, never re-braced) and can appear as a forward-map value without matching that shape. Missed names extend the mapping, hallucinated names drop referencing triples. Before anything is forwarded to SOTA, an anonymizer-stage predicate invariant additionally drops any fact whose `predicate` field contains a token from the declared placeholder vocabulary (the anonymizer's `reverse` map) — a placeholder glued into a predicate (e.g. `language_proficiency_Language_3`) is dropped outright, never sent to the cloud.
