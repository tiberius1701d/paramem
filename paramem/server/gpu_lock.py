...
def gpu_lock_released():
    ...
    Use only from code paths that statically hold the reservation. This
    primitive has no production callers; it exists as a symmetric
    counterpart to ``gpu_lock_sync``.
    """
    release_gpu()
    try:
        yield
