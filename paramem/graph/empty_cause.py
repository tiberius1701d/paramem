...
#
# Both kinds are recorded only — neither
# :func:`~paramem.graph.relation_build.recovery_gate` nor the empty-result
# terminal nor the fallback distinguishes between them.
# ---------------------------------------------------------------------------
...
def cause_kind(cause: str | None) -> str:
    """Classify an ``EMPTY_CAUSE`` value as judgment / breakage.

    See the vocabulary comment above for the two-way distinction.
    ...
    Returns:
        ``"judgment"``, ``"breakage"``, or ``"unattributed"`` for ``None``
        / any value outside the vocabulary.
    """
    if cause is None:
