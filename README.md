...
- **Pipeline:** privacy-aware extraction (local extract → anonymize → cloud enrichment with explicit binding → de-anonymize via state-machine substitution → plausibility), graph-level cloud enrichment at full consolidation, anti-confabulation voice prompt, deferred identity binding with BPE-stable `speaker{N}` placeholders.
...
├── evaluation/       # Recall metrics, embedding scoring, fidelity, RAG baseline
