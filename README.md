...
Background training stops at the next step boundary so an inference request is never queued behind a full epoch; the interrupted cycle is retried later rather than committed. It also checkpoints at every epoch boundary so a crash mid-cycle resumes from the last completed epoch rather than restarting from zero.
