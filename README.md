...
- **Pipeline:** 7-stage privacy-aware extraction (local extract → anonymize → SOTA enrichment with explicit binding → de-anonymize via state-machine substitution → plausibility → transcript-grounding gate), graph-level SOTA enrichment at full consolidation, anti-confabulation voice prompt, deferred identity binding with BPE-stable `Speaker{N}` placeholders.
