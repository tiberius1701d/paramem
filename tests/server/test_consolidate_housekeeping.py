...
class TestRunFullConsolidationSyncHousekeeping:
    ...
    def test_run_housekeeping_called_not_consolidate_interim_adapters(
        ...
        consolidate_interim_adapters or consolidate_simulate_fold directly."""
        ...
        mock_loop.consolidate_simulate_fold.assert_not_called()
...
class TestRunHousekeepingModeParam:
    ...
    - ``mode="simulate"`` routes to ``consolidate_simulate_fold(housekeeping=True)``.
    - Default (no mode arg) routes to simulate — safe when no GPU is available.

    Both ``consolidate_interim_adapters`` and ``consolidate_simulate_fold`` are
    ...
    def _make_bare_loop(self, tmp_path) -> "object":
        ...
        loop.consolidate_simulate_fold = MagicMock(
            return_value={
                "tiers_rebuilt": [],
                "graph_drift_count": 0,
                "drift_deduplicated": 0,
    ...
    def test_train_mode_dispatches_to_consolidate_interim_adapters(self, tmp_path) -> None:
        ...
        The simulate branch (consolidate_simulate_fold) must NOT be called.
        """
        ...
        loop.consolidate_simulate_fold.assert_not_called()

    def test_simulate_mode_dispatches_to_consolidate_simulate_fold(self, tmp_path) -> None:
        """mode="simulate" calls consolidate_simulate_fold(housekeeping=True).
        ...
        loop.consolidate_simulate_fold.assert_called_once_with(housekeeping=True)
