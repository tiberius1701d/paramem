...
def test_consolidation_count_zero_with_valid_cadence_accepted():
    """max_interim_count=0 + a valid refresh_cadence ('12h') is accepted, and
    the derived period equals the cadence's own seconds -- at count=0 the
    cadence itself is the full-fold schedule, not cadence × count."""
    cfg = ConsolidationScheduleConfig(max_interim_count=0, refresh_cadence="12h")
    assert cfg.consolidation_period_seconds == 12 * 3600


@pytest.mark.parametrize("cadence", ["", "off"])
def test_consolidation_count_zero_with_off_cadence_refused(cadence: str) -> None:
    """max_interim_count=0 requires a non-empty refresh_cadence -- at count=0
    the full fold is the only training venue and needs a cadence to run on."""
    with pytest.raises(ValueError, match="refresh_cadence"):
        ConsolidationScheduleConfig(max_interim_count=0, refresh_cadence=cadence)
