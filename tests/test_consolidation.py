...
The full-fold deadline and ``_oldest_interim_stamp`` are covered through the
dispatch integration tests in ``tests/server/test_consolidate_dispatch.py``
and the decider's own table in ``tests/server/test_consolidation_choice.py``
...
class TestTickGateNoNamed:
    ...
    def _call_tick(self, state_overrides: dict) -> str:
        ...
        # and _retro_claim_orphan_sessions to no-op; the stale cadence mark the
        # state builder seeds makes the tick resolve to INTERIM.
        with (
            ...
            status, _action = _app._dispatch_consolidation(
                _app.ConsolidationAction.AUTO, reason=DispatchReason.TIMER
            )
            return status
