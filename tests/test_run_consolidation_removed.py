"""Regression guard: run_consolidation must not exist in consolidation.py (D2).
...
def test_run_consolidation_not_in_consolidation_module():
    """paramem.server.consolidation must not export run_consolidation (D2)."""
    import paramem.server.consolidation as _m

    assert not hasattr(_m, "run_consolidation"), (
        "run_consolidation was deleted in the voice-pipeline-switch commit (D2). "
...
def test_run_extraction_phase_exists_in_app():
    """paramem.server.app must export _run_extraction_phase (D2 replacement)."""
    import paramem.server.app as _app

    assert hasattr(_app, "_run_extraction_phase"), (
        "_run_extraction_phase is the D2 replacement for run_consolidation. "
