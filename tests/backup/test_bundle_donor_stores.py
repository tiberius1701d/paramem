...
def _seed_donor_store(adapters_root: Path, *, manifest_base_repo: str = _BASE_REPO) -> Path:
    """Write a donor store under *adapters_root*; return the store dir.

    Valid for seeding by default. ``manifest_base_repo`` selects the
    ``base_model.repo`` the manifest records the slot was trained under;
    pass a value other than ``_BASE_REPO`` to make the slot fail the
    seeding gate's base-model check while its store path (keyed on
    ``_BASE_REPO``) is left unchanged.

    The manifest is built through the production dataclasses rather than a
    hand-written dict so a field added to the schema breaks this fixture at
    construction instead of silently defaulting it to a legacy value.
    """
    ...
    write_manifest(
        ...
        AdapterManifest(
            ...
            base_model=BaseModelFingerprint(
                repo=manifest_base_repo, sha="c170c708abc", hash="sha256:f4b6abc"
            ),
...
def _bundle_with_donor(
    tmp_path: Path, *, manifest_base_repo: str = _BASE_REPO
) -> tuple[Path, dict, Path]:
    src = _make_source_store(tmp_path)
    adapters_root = next(iter(src["adapter_dirs"].values())).parent
    store = _seed_donor_store(adapters_root, manifest_base_repo=manifest_base_repo)
    slot = write_bundle(
...
class TestDonorStoreInBundle:
    ...
    def test_restore_keeps_a_donor_it_cannot_seed_from(self, tmp_path) -> None:
        """Invalid-for-seeding is not delete-on-restore.

        A donor whose manifest records a different base model than the one
        its store path is keyed on fails the seeding gate, and the funnel
        rebuilds instead of seeding from it — but restoring a bundle that
        carries one must still put its bytes back untouched. Refusing a
        seed is the funnel's decision; destroying an artifact the operator
        asked to be restored is not restore's.
        """
        bundle_slot, _src, store = _bundle_with_donor(
            tmp_path, manifest_base_repo="mistralai/Mistral-7B-Instruct-v0.2"
        )
        scratch = tmp_path / "scratch"
        scratch.mkdir()
        ...
        restored = scratch / "adapters" / store.name
        assert donor_checkpoint_valid(restored, _BASE_REPO, _LORA_SHAPE) is False
        slots = [p for p in restored.iterdir() if p.is_dir()]
        assert len(slots) == 1
        assert slots[0].joinpath("adapter_model.safetensors").read_bytes() == _DONOR_WEIGHTS
