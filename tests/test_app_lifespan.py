...
Also covers:
5. ConsolidationScheduleConfig.training_idle_debounce_s field validation.
6. _maybe_trigger_scheduled_consolidation idle-debounce gate.
...
# ---------------------------------------------------------------------------
# TestPostSessionEnqueueIdleDebounce — enqueue_post_session_train gate
# ---------------------------------------------------------------------------


def _make_enqueue_config(debounce_s: int = 30) -> MagicMock:
    """Build a minimal config mock for enqueue debounce tests."""
    cfg = MagicMock()
    cfg.consolidation.training_idle_debounce_s = debounce_s
...
class TestPostSessionEnqueueIdleDebounce:
    """enqueue_post_session_train skips submit() when within the debounce window."""

    def test_enqueue_deferred_within_debounce_window(self) -> None:
        """submit() is NOT called when /chat fired 5 s ago with debounce=30."""
        from paramem.server.inference import enqueue_post_session_train

        mock_bt = MagicMock()
        mock_bt.submit = MagicMock()
        state = {"last_chat_monotonic": time.monotonic() - 5}

        enqueue_post_session_train(
            conversation_id="conv-debounce-001",
            transcript="hello",
            speaker_id="spk-001",
            speaker_name="Alice",
            loop=MagicMock(),
            background_trainer=mock_bt,
            config=_make_enqueue_config(debounce_s=30),
            state=state,
        )

        mock_bt.submit.assert_not_called()

    def test_enqueue_proceeds_after_debounce_elapsed(self) -> None:
        """submit() IS called when /chat fired 60 s ago with debounce=30."""
        from paramem.server.inference import enqueue_post_session_train

        mock_bt = MagicMock()
        mock_bt.submit = MagicMock()
        state = {"last_chat_monotonic": time.monotonic() - 60}

        enqueue_post_session_train(
            conversation_id="conv-debounce-002",
            transcript="hello",
            speaker_id="spk-001",
            speaker_name="Alice",
            loop=MagicMock(),
        ...
        mock_bt.submit.assert_called_once()

    def test_enqueue_proceeds_when_state_missing(self) -> None:
        """submit() IS called when state has no last_chat_monotonic key."""
        from paramem.server.inference import enqueue_post_session_train

        mock_bt = MagicMock()
        mock_bt.submit = MagicMock()
        state: dict = {}  # no last_chat_monotonic

        enqueue_post_session_train(
            conversation_id="conv-debounce-003",
            transcript="hello",
            speaker_id="spk-001",
            speaker_name="Alice",
            loop=MagicMock(),
    ...
    def test_enqueue_proceeds_when_debounce_zero(self) -> None:
        """debounce_s=0 disables the gate even when /chat fired just now."""
        from paramem.server.inference import enqueue_post_session_train

        mock_bt = MagicMock()
        mock_bt.submit = MagicMock()
        state = {"last_chat_monotonic": time.monotonic()}  # just now

        enqueue_post_session_train(
            conversation_id="conv-debounce-004",
            transcript="hello",
            speaker_id="spk-001",
            speaker_name="Alice",
            loop=MagicMock(),
            background_trainer=mock_bt,
            config=_make_enqueue_config(debounce_s=0),
            state=state,
        )
