...
class TestCapturePendingRelationsAttributes:
    """``_capture_pending_relations`` must recover attribute-typed facts
    (``relation_type == "attribute"``) via a node-attribute walk, not just
    edges -- ``GraphMerger.merge`` diverts an attribute-typed relation onto
    the subject node's ``attributes`` dict and never inserts an edge for it,
    so an edge-only capture drops it the moment ``stage_event`` resets the
    graph.
    """

    def test_pending_attribute_and_factual_relations_both_reach_keying(self, tmp_path):
        """The production route: a session graph carrying one attribute-typed
        and one factual relation is merged into ``merger.graph``, captured by
        ``_capture_pending_relations`` (as ``stage_event``'s caller does
        before the reset), and both facts are keyed by the phase-1 build."""
        ...
        assert {r.relation_type for r in pending} == {"factual", "attribute"}
        ...
        result = loop.stage_event(
            ...
            stamp="stamp1",
            primary_tiers={"episodic": "episodic"},
            episodic_rels=pending,
        )
        assert result is not None
        keyed = json.loads((_shadow_dir(loop, "interim", "episodic") / "keyed.json").read_text())
        assert len(keyed) == 2
        rtypes = {row["relation_type"] for row in keyed}
        assert rtypes == {"factual", "attribute"}
        objects = {row["object"] for row in keyed}
        assert objects == {"berlin", "alex@example.com"}

    def test_recaptured_attribute_dedups_rather_than_re_keys(self, tmp_path):
        """A second observation of an already-keyed attribute fact must
        replay the existing key via recall, not mint a new one."""
        loop = _make_loop(tmp_path)
        loop.store.registry("episodic").add("graph1")
        loop.store.set_bookkeeping(
            "graph1",
        ...
        next_index_before = loop._indexed_next_index

        # Re-observe the same fact in a fresh (not-yet-keyed) session merge.
        from paramem.graph.schema import Entity, SessionGraph
        ...
        assert pending[0].relation_type == "attribute"
        ...
        keyed = json.loads((_shadow_dir(loop, "full", "episodic") / "keyed.json").read_text())
        assert [row["key"] for row in keyed] == ["graph1"]
        assert loop._indexed_next_index == next_index_before
