...
Adapter weights are the dominant artifact by volume and sensitivity. They are numerical — not directly readable for facts — but also not probe-resistant. They are kept **plaintext at rest** by design; the rationale is in §4 (carve-out) and §7 (probe limit).
...
- `data/ha/adapters/<tier>/<slot>/adapter_model.safetensors` (and the paired `adapter_config.json`) — LoRA weight tensors. Plaintext at rest **by design**, regardless of Security posture. Rationale: the systematic-extraction gate is the SimHash registry (`<store>/<tier>/keyed_pairs.json`, encrypted under Security ON). Without the registry an attacker has no way to enumerate the keys that the indexed-recall template requires, so the keyed-recall path is closed by encrypting the registry alone. Encrypting the weight tensors as well would be defense-in-depth against blob-copy attackers, but it does not narrow the actual probing surface — see §7 — and adding the encrypt-on-save / decrypt-on-load lifecycle to PEFT's `save_pretrained` / `from_pretrained` is non-trivial work for marginal gain. Weights remain numerical — `.safetensors` files are opaque to grep but transparent to a model with the right prompts.

None of the first three carry user facts. The next two *do* carry user facts but are produced only at operator request via the `debug` flag. Adapter weights carry user facts as numerical patterns; encrypting them does not block the probing surface that already governs how the system is queried.
...
- **Plaintext weights at rest, encrypted registry.** Following from the limit above, ParaMem encrypts the SimHash registry (`<store>/<tier>/keyed_pairs.json`) but leaves the LoRA weight tensors plaintext. The registry encryption blocks the **systematic** extraction path: the indexed-recall template requires knowing the key string (`graph17`, `proc4`, …), and without the registry an attacker cannot enumerate keys. What remains accessible to a weights-only attacker is targeted natural-language probing ("what did Alex say about X?"), membership inference, and continued fine-tuning — none of which are blocked by encrypting weights at rest, because all of them require running inference on the weights anyway. The architectural call: encrypt the gate (registry), accept that the gated material (weights) is opaque-but-not-defended. Operators who need defense-in-depth against blob-copy theft of the data directory should rely on filesystem-layer encryption (LUKS, EncFS) rather than per-file wrapping.
