...
- **Extraction-stage SOTA enrichment narrows but does not eliminate PII egress, and completeness cannot be verified.** When `consolidation.extraction_noise_filter` is set to a SOTA provider (default `""` = disabled), the extraction pipeline runs a local-anonymization pass — the SAME local model that ran extraction, on a second prompt — replacing in-scope names with type-tagged placeholders (e.g. `Person_1`) before sending the anonymized transcript and facts to the cloud for coreference resolution, compound splitting, and dedup. This is single-model classification: there is no independent second opinion in the default configuration, because extraction decided what counts as a person and anonymization decided what to mask, and both decisions came from the same weights. There is **no forward-path leak check** on this payload: substitution is case-**sensitive**, and that is load-bearing. Case is the only signal separating a person named `Bill` from the common noun `bill` (an invoice), or `Will`/`will`, `Mark`/`mark`, `Rose`/`rose`. Any case-insensitive match over entity names cannot tell a real leak from ordinary prose — it would fire on the common noun for any entity-derived name and silently disable cloud enrichment for that user on every session — so the pipeline never performs one. What substitution DOES guarantee mechanically: every name `_build_anonymization_mapping` walked — every in-scope `graph.entities` name plus its PII attribute values — is replaced exactly, case-sensitively, with no exceptions; there is nothing left for a post-hoc scan to verify at that boundary. The residual, stated plainly, in three parts: **(a)** a real-name surface in the transcript that differs in **case** from its `graph.entities` surface egresses in the clear — catching this would require a case-insensitive match, which is exactly the mechanism ruled out above, so nothing covers this class. **(b)** An undeclared relation participant — a name that appears only as a fact's subject/object and was never surfaced as its own `graph.entities` record (e.g. `Honda` in a `(Alex, drives, Honda)` triple when `Honda` was never typed as an entity) — is covered only if the anonymizer LLM's own mapping hint happened to name it. **(c)** A name the local model typed as an out-of-scope entity type (a person typed as an organization, discussed below) never reaches the table at all and egresses verbatim. (a), (b), and (c) are all structurally invisible to a forward-path check built on an exact, case-sensitive substitution table — that is the accepted tradeoff for a check that does not misidentify ordinary prose as a leak. The local model can also simply *omit* a name it should have classified — in whole or in part. The empty-mapping guard described next only catches the case where a chunk's mapping comes back **totally** empty; a mapping that names 2 of 10 in-scope persons and silently drops the other 8 passes that guard, and the remaining 8 names egress verbatim — a partial omission is invisible to every check in this pipeline, not merely to the ones documented as residual below. What *is* guaranteed mechanically: given a mapping whose keys match the surfaces present, substitution is exact and complete. At the graph tier (below), the local model's mapping keys are surface strings it produced independently of the fold graph's own canonical node-key text, so before substitution ever runs each key is reconciled to the graph's node identities via `canonical()` — a re-cased, separator-varied, or diacritic-varied key still resolves against the node it names; an entry that matches no node in its chunk (or an ambiguous multiple) is dropped rather than guessed. Session-tier substitution never needed this reconciliation (its facts are never built from pre-canonicalized node-key text) and its identity matching stays exact throughout. Separately, an anonymizer mapping that comes back completely empty — or, at the graph tier, one whose every entry the reconciliation above drops — for content that needed classifying is treated as a detected failure and blocks the call, rather than being silently accepted as "nothing in scope." The residual — an entity the model misclassifies (e.g. a person typed as an organization) — is sent to the cloud in the clear, and this cannot be detected or undone once it has egressed. That residual is why the ship default is `sota_enabled: false` (`refinement_enrichment: "off"` for the graph-tier pass below): the operator's only hard guarantee is the off switch. The default cloud-egress scope is `{"person"}`; place names, organization names, phone numbers, email addresses, and free-form secrets (API keys, passwords, tokens) flow to the SOTA provider inside the anonymized payload. The scope is configurable via `sanitization.cloud_scope`, but broadening it strips the structured context a personal assistant relies on to be useful — privacy-vs-utility is the operator's call, made consciously and per deployment. Document ingestion goes through the same pipeline; for documents that may contain machine credentials, keep `extraction_noise_filter=""` or scrub credentials before ingest. **The graph-tier enrichment pass** (`ConsolidationLoop._run_graph_enrichment`, the post-merge cross-session second-order-relation pass) runs the SAME anonymize → SOTA → de-anonymize contract, through the same shared primitives, over the cumulative cross-session graph — it is not a separate, unprotected cloud call, and it carries the identical single-model-classification limitation described above. Unlike session-tier extraction, the cumulative fold graph carries no reliable entity types of its own (registry-derived relations have none), so before each chunk's SOTA call this pass runs the SAME local-model anonymizer session-tier extraction uses and derives each real name's type from the placeholder the local model minted for it; a local mapping that comes back completely empty — or whose every entry the `canonical()` node-key reconciliation above drops — for a chunk with real (non-speaker) content is treated as a classification failure and that chunk's cloud call is skipped (fail-closed) rather than sent unmasked. This is the same TOTAL-omission guard as the extraction stage, with the identical partial-omission gap: at this tier there is no independent type source at all (the fold graph carries no reliable entity types of its own), so a name the local model partially omits is entirely invisible here too. Under the default `{"person"}` scope, the anonymous `speaker{N}` handle is never tokenised at this tier either (the local anonymizer prompt forbids mapping it, matching the session tier) — it reaches the payload bare by design, since it carries no identifying information. Accepted consequence: person-level `same_as` coreference (recognizing two surface forms of the same person, e.g. a nickname or honorific variant) can no longer be detected by the cloud judge once both surfaces are opaque tokens — the name-surface signal that coreference depends on is gone for people under the default scope. Coreference for organizations, places, and other non-person entities is unaffected (those surfaces stay verbatim under the default scope).
