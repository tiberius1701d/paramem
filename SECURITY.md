...
- **Deciding `cloud.allow_degraded_serving`.** When the local model becomes unavailable for a reason the operator did not choose (GPU held by another process, insufficient VRAM, a failed adapter reload/apply, a persistent CUDA fault), the ship default `false` closes the cloud leg: HA still answers, and anything HA cannot serve returns a canned limited-mode reply. Setting it `true` accepts that the household's questions route to a third party during an outage — with the same `cloud_mode` policy as normal operation, but without the local memory that would otherwise have answered them.
