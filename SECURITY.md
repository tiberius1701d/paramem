...
None of the first three carry user facts. The next two *do* carry user facts but are produced only at operator request via the `debug` flag. Adapter weight blobs carry user facts as numerical patterns; see §8 for the probe-resistance limit that encryption does not fully close.
