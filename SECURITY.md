...
- **Extraction-stage SOTA enrichment narrows but does not eliminate PII egress, and completeness cannot be independently verified.** When `consolidation.extraction_noise_filter` is set to a SOTA provider (default `""` = disabled), the extraction pipeline runs a local-anonymization pass — the SAME local model that ran extraction, on a second prompt — before sending an anonymized transcript and fact array to the cloud for coreference resolution, compound splitting, and dedup. The local model is the **sole classifier**: given the operator's `sanitization.scrub` vocabulary (default: person name, email address, phone number, physical address, social/professional profile URL), it decides per value whether it is in scope, mints a type-tagged placeholder for it (e.g. `Person_1`), and returns that `{real → placeholder}` mapping. There is no code-side entity-type gate downstream of that decision, and no second, independent classifier — this is a deliberate design choice, argued below, not an oversight.
  ...
  Free-form secrets (API keys, passwords, tokens) are not a `scrub` category and are not scrubbed by this pass. Place names and organization names are deliberately left verbatim under the default `scrub` so the cloud can still reason about them (e.g. "What's a good restaurant in Berlin?"). Document ingestion goes through the same pipeline; for documents that may contain machine credentials, keep `extraction_noise_filter=""` or scrub credentials before ingest. `scrub` is configurable via `sanitization.scrub`; narrowing or broadening it is a privacy-vs-utility tradeoff the operator makes consciously, per deployment. That residual is why the ship default is `sota_enabled: false` (`refinement_enrichment: "off"` for the graph-tier pass below): the operator's only hard guarantee is the off switch.

  **The graph-tier enrichment pass** (`ConsolidationLoop._run_graph_enrichment`, the post-merge cross-session second-order-relation pass) runs the SAME anonymize → SOTA → de-anonymize contract, through the same shared primitives, over the cumulative cross-session graph — it is not a separate, unprotected cloud call, and it carries the identical single-model-classification limitation described above. The cumulative fold graph carries no reliable entity types of its own (registry-derived relations have none), and this pass does not attempt to derive one: before each chunk's SOTA call it runs the SAME local-model anonymizer session-tier extraction uses, scoped against the same `scrub`, and threads its mapping straight through to the cloud call rather than re-deriving or re-typing it. Because the local model's mapping keys are surface strings it produced independently of the fold graph's own canonical node-key text, each key is first reconciled to the graph's node identities via `canonical()` — a re-cased, separator-varied, or diacritic-varied key still resolves against the node it names, with the model's own placeholder preserved verbatim; an entry that matches no node in its chunk (or an ambiguous multiple) is dropped rather than guessed. This tier fails closed only when the local anonymizer itself fails to parse (mirroring the extraction stage's guard); a local mapping that comes back empty is a legitimate "nothing in scope" verdict and proceeds, exactly like the extraction stage. The one additional guard here is narrower: when the local mapping names something but reconciliation onto this chunk's actual node keys drops all of it for a chunk with real (non-speaker) content, that residual is a classification/identity failure rather than evidence of out-of-scope content, and the chunk's cloud call is skipped (fail-closed) rather than sent unmasked. Neither guard is a totality check: a chunk in which the model names only some of the real content, or whose mapping only partially survives reconciliation, still passes, and the unnamed/dropped names egress verbatim. The anonymous `speaker{N}` handle is never tokenised at this tier either (the local anonymizer prompt forbids mapping it, matching the session tier) — it reaches the payload bare by design, since it carries no identifying information. Accepted consequence: person-level `same_as` coreference (recognizing two surface forms of the same person, e.g. a nickname or honorific variant) can no longer be detected by the cloud judge once both surfaces are opaque tokens — the name-surface signal that coreference depends on is gone for people under the default `scrub`. Coreference for organizations, places, and other non-person entities is unaffected (those surfaces stay verbatim under the default `scrub`).
