...
The anonymized facts and the anonymized transcript cloud sees are both built entirely by the pipeline in step 2, from the one table it mints and owns, substituted through the same exact, case-sensitive primitive — there is nothing left for a later stage to re-verify on either surface; whatever the local model did not name as in scope is checked offline, at the calibration gate (see SECURITY.md). What needs a runtime check is what cloud sends *back* (step 5), which is a model rewriting content, not a table the pipeline owns.
