...
| Option | Pros | Cons | Decision |
|--------|------|------|----------|
| Resident local language model | Reads the whole sentence, not just a span; takes the operator's list of kinds as plain-language definitions; expected to improve with a stronger base model — the operator's own test tool measures a change against the last accepted scorecard when run — with no separate model to maintain | Slower per turn than a span tagger; trails a tuned span tagger in measured accuracy on this task; the anonymizer has no local detector at all when the base model is not loaded, so under an anonymizing cloud policy the cloud leg refuses rather than sending unmarked text | **Chosen** |
...
A local mapping that comes back completely empty is a legitimate "nothing in scope" verdict, and the chunk's whole fact list proceeds to the cloud call. The chunk is held back only when the local model marked at least one value of an in-scope kind, the chunk's facts name someone other than the speaker, and none of the marked values survives both matching onto the chunk's own node identities and the check that it occurs in the outbound fact text. A value marked under a kind the operator does not scrub, a keyword the table does not carry, or the speaker's own handle is set aside before this check and never triggers it. When it does fire, the whole chunk's cloud call is skipped, never a partial subset of the chunk's facts — the next chunk is still tried. Under the default `sanitization.scrub` (person name, email address, phone number, postal address, social profile URL), the anonymous `speaker{N}` handle is never tokenised at this tier either: the same marking pass session-tier extraction uses runs here too, but the step that turns the model's marked values into kept and restored values drops any speaker-shaped value on sight, before it can be kept under any keyword, so the handle reaches the payload bare regardless of what the model marks it as. **Operator opt-out**: an explicitly empty `sanitization.scrub` short-circuits before any model call — the chunk's triples egress to the cloud VERBATIM, the same opt-out contract every other cloud-egress path honours (see SECURITY.md for the privacy-posture note).

When the local model is not resident at all, this pass is skipped entirely before any chunk is touched — there is nothing to keep, since nothing was merged. Once the pass has started, a local resource fault (insufficient free VRAM) or a marking call that fails outright degrades the pass, not just the one chunk in progress: the pass stops processing further chunks, keeps whatever chunks it already merged, and returns normally rather than raising — the chunk in progress when the fault occurred contributes nothing, but chunks that already completed keep their enrichment relations. An operator-visible incident naming the cause is recorded and the fold proceeds to train on the merged-but-unenriched graph; enrichment self-heals at the next **full** fold — this pass is full-fold only (see AD-15 below), so recovery does not happen at an intervening interim cycle. The incident clears itself on that recovery: an enrichment pass that runs to completion resolves it, so the attention row on `/status` reflects the current state rather than the worst state ever reached. When the operator has cloud egress disabled entirely, a completed pass can never happen — so the incident instead clears the next time a session is processed, carrying a recorded reason that distinguishes "resolved by a clean run" from "resolved because cloud is off," rather than riding on `/status` forever with no path back to green.
...
every path and carries the turn to the configured HA agent exactly as
written — no scrub, no placeholders — regardless of the turn's verdict.
...
point of escalation; a self-referential forwarded query skips the HA hop,
because `ha_agent_id` is operator-pointed and may be cloud-backed. Its cloud
hop still follows `cloud_mode` exactly as any personal turn does: anonymized
under `anonymize`, refused outright under the shipped default `block` and
under `both` alike.
...
cloud — share one outbound-text object, with different policies: the cloud
door applies `cloud_mode` (`block`/`anonymize`/`both`) exactly as configured,
scrubbing and restoring through the anonymize chain; the HA door sends the
turn as written — no scrub, no placeholders, no restore — and is never
closed by the personal verdict or refused for one; it simply does not
answer when no HA client or agent is configured. Each door reads the same
per-outbound-text object built once per turn, and the anonymize chain runs
at most once per outbound text, so an HA-miss →
cloud-fallback turn's cloud leg is scrubbed by that single run. Forced routing is a probe
...
applies `cloud_mode` in every residency state: a personal query is still
refused under `block`/`both` regardless of whether the model is resident.
Under `anonymize`/`both`, marking needs the resident model exactly as the
rest of the anonymize chain does: with no model resident, the turn is
refused rather than sent unscrubbed, because the marking call the chain
depends on cannot run. On a cloud-only deferral the memory store is absent, so no
