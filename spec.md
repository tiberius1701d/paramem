...
- **F5.1a:** ParaMem server — standalone HTTP daemon wrapping the inference pipeline. Core endpoints: `/chat`, `/status`, `/consolidate`, `/ingest-sessions`, `/refresh-ha`, `/gpu/{acquire,release}`. Operator/dev surface (gated): `/admin/assign-orphans` (debug=true; permanent), `/debug/probe` (debug=true; ephemeral), `/debug/recall` (debug=true; direct adapter probe), `/debug/dump` (debug=true; in-memory store dump), `/calibrate/{extract,anonymize,plausibility}` (calibrate_endpoint_enabled=true). Subsystem surfaces: `/migration/*`, `/backup/*`. Model loaded once at startup, adapter always active. **IMPLEMENTED** — `paramem/server/app.py`, `inference.py`, `config.py`, `escalation.py`, `session_buffer.py`. Architecture decision: external service (not HA add-on) — crash isolation, independent lifecycle, GPU on separate hardware.
...
- **F5.4a:** Registry encryption — *superseded by encrypted infrastructure store (WP1 in security hardening plan).* Scope expanded beyond the registry to cover all infrastructure metadata (registry, graph, queue, snapshots, speaker store, resume state, manifest sidecars, migration backups) under a uniform age two-identity model. Daily identity passphrase-wrapped on disk; recovery identity printed once at `paramem generate-key` time. Technology: age / pyrage. See internal `docs/plan_security_hardening.md`.
- **F5.4b:** ~~Registry hashing~~ — *RETRACTED 2026-04-22.* Structurally incompatible with training reliability. Empirical evidence: `benchmarking.md:1228–1238` — key ID layout (sequential `graphN` tokens) is load-bearing for adapter convergence. Interleaving new keys into old ID-space already causes epoch-budget pressure in 4/34 cycles; hashing would destroy sequential structure entirely and make every cycle borderline. Either training keys change (breaks training) or only the external identifier changes (collapses to file encryption with extra steps). No standalone hardening lever here.
...
5. **Adapter merging vs. switching:** *Closed — switching required.* Test 7b showed linear adapter merging destroys indexed key recall at rank 8 (0/50). The structured key→QA mapping needs token-level precision that weight averaging destroys. Superseded operationally by `set_adapter()` switching + multi-adapter interim routing (F5.8). Higher-rank merge tolerance is moot given the chosen architecture.
7. **Adapter-active extraction:** *Closed — refuted.* Test 11 (benchmarking §Test 11, ~2026-03): adapter OFF extracts 15.2 triples/session at 98% entity grounding; adapter ON drops to 12.4 at 92%. The novelty-filter hypothesis does not hold; adapter remains disabled during extraction. (resolved 2026-04-22)
8. **Small model retrieval, large model reasoning:** *Closed — architecturally covered.* Qwen 2.5 3B validated at 100% recall through 100 keys (Test 1). Multi-adapter interim routing (F5.8) ships the separation pattern in production. Specific Qwen-retrieve + larger-reason orchestration benchmark is not run, but the mechanism is proven viable. (resolved 2026-04-22)
9. **Training format hardening for probe resistance:** *Closed — subsumed by F5.4.* Negative-example refusal training is not pursued as a standalone lever; probe resistance is tracked under the Security Hardening section (F5.4a–e): registry encryption, registry hashing, vocabulary permutation, adapter weight encryption, extraction-resistance validation. (resolved 2026-04-22)
10. **Continuous online learning:** *Closed — negative.* Test 4b showed that training only new keys without full replay causes catastrophic forgetting (0–4 of 20–40 old keys survive at rank 8). One-gradient-step-per-fact would inherit the same failure mode. Batch consolidation with full replay remains mandatory; multi-adapter interim routing (F5.8) is the production pattern for mid-window recency. (resolved 2026-04-22)
12. **Rephrased question diversity:** *Closed — validated.* Test 10b (benchmarking §Test 10b) exercised 5 styles × 129 keys: Indirect 89.7%, Contextual 67.3%, Partial 56.5%, Formal 56.0%, Colloquial 44.7%. Indirect recall at ~90% is the headline; colloquial is the hardest style. (resolved 2026-04-22)
