...
- **F5.1a:** ParaMem server — standalone HTTP daemon wrapping the inference pipeline. Core endpoints: `/chat`, `/status`, `/consolidate`, `/ingest-sessions`, `/refresh-ha`, `/gpu/{acquire,release}`. Operator/dev surface (gated): `/admin/assign-orphans` (debug=true; permanent), `/debug/probe` (debug=true; ephemeral), `/calibrate/{extract,anonymize,plausibility}` (calibrate_endpoint_enabled=true). Subsystem surfaces: `/migration/*`, `/backup/*`. Model loaded once at startup, adapter always active. **IMPLEMENTED** — `paramem/server/app.py`, `inference.py`, `config.py`, `escalation.py`, `session_buffer.py`. Architecture decision: external service (not HA add-on) — crash isolation, independent lifecycle, GPU on separate hardware.
