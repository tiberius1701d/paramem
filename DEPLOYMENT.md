...
| POST | `/speaker/forget` | admin | Remove a speaker's profile, erase their indexed-memory keys — cached content, per-tier registry and SimHash entries, provenance, and the underlying stored facts — and discard pending sessions. Does not trigger retraining; a resident tier's weights are trimmed only by a later operator-invoked `POST /reconsolidate`. See [Forgetting a speaker](#forgetting-a-speaker-data-erasure). |
