...
- **Local anonymizer's detection model.** On the first start of a
  configuration with an external-egress path that needs scrubbing — a cloud
  leg (see `sanitization` and `span_tagger` under
  [Configuration](#configuration) below) or an HA leg, since it scrubs
  unconditionally — the local
  PII-detection model is fetched and
  cached the same way as the conversational base model, and brings a second
  cache entry for its own encoder's tokenizer. An offline host preparing this
  configuration must warm both ahead of time; an egress-capable deployment
  that cannot resolve either at start refuses to start rather than boot with
  no detector.
...
to handle correctly and the near-miss values it must leave alone.
`scripts/dev/calibrate_cloud_anonymizer.py` loads this fixture by
default when exercising `/calibrate/anonymize`, so a calibration run can
be repeated and compared against an earlier one without depending on any
...
- Non-memory queries are scrubbed under `sanitization.scrub` and forwarded back to HA's configured conversation agent via WebSocket (`conversation.process`), which handles device control, search, weather, and other tools with full room awareness; entity and area names Home Assistant itself registered stay readable so device control still works
