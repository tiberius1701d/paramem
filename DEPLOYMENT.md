...
| POST | `/reconsolidate` | admin | Rebuild main memory from its own stored knowledge — runs even when nothing is new, and leaves the interim slots and pending conversations untouched. Use after changing the base model, the extraction prompts, the extraction config, or the prompt/tokenization format. |
