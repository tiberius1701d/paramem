...
- **Registry/bookkeeping integrity gate:** a full consolidation cycle fails closed if the trained key registry and the key bookkeeping metadata disagree — the previous memory state stays live and servable, and the cycle records an incident naming the divergent keys. Because nothing was consumed, the same divergence keeps failing on retry until the named keys are erased via `POST /debug/erase-keys` (or the divergence is otherwise resolved). The boot log also reports a count of registry keys that have no bookkeeping record.
...
| `POST /reconsolidate` | Rebuild main memory from **its own** stored knowledge. The interim slots are neither folded in nor reaped and the pending conversations stay pending, so nothing is lost by running it. Use after changing the base model, the extraction prompts, the extraction config, or the prompt/tokenization format. |
...
**A refusal is not an error.** These endpoints are non-blocking: they submit the run and return immediately. When the server is busy (another fold running, someone chatting, the GPU held, cloud-only mode) the response is still **200** with `status: "deferred_*"`; when there is nothing to do it is **200** with `status: "noop_*"`. `curl --fail` therefore does **not** exit non-zero on a deferral — read `status`. The only 4xx from this family is **409 `trial_active`** while a migration TRIAL is in progress. All four endpoints — `POST /reconsolidate` included — also defer with `status: "deferred_tier_unverified"` while a main memory tier's on-disk state cannot be verified against its adapter slots; the recovery step is restoring the affected tier from a snapshot bundle (`POST /backup/restore`) and restarting. Forgetting a speaker (`POST /speaker/forget`), erasing individual keys (`POST /debug/erase-keys`), and discarding the interim ring (`POST /interim/discard`) all keep working while consolidation is deferred this way.
...
| POST | `/reconsolidate` | admin | Rebuild main memory from its own stored knowledge — dispatches even when there is no new interim content or pending conversation, and leaves the interim slots and pending conversations untouched. Noops (`noop_no_stored_keys`) when no main tier holds an active key. Use after changing the base model, the extraction prompts, the extraction config, or the prompt/tokenization format. |
...
| POST | `/debug/erase-keys` | admin + `config.debug=true` | Operator-only repair tool: permanently hard-erases an explicit list of memory keys — the sanctioned way to resolve a registry/bookkeeping divergence (see [Consolidation & Crash Safety](#consolidation--crash-safety)). Requires `{"confirm": true}`; without it, refuses with **409** and previews the keys that would be erased. Refuses with **409** while busy, training, TRIAL, or cloud-only. |
