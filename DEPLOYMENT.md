...
| POST | `/chat` | chat | Send a conversation turn; returns the assistant reply. Speaker is resolved from the bearer token (attributed per-user token) or voice embedding (unattributed token). See curl example below. |
