...
| `POST /scheduled-tick` | The systemd user-timer entrypoint — the only door that lets the schedule's deadline math (`_is_full_cycle_due`) decide between a full fold and an interim absorb, and the only one with the catch-up gate and the cadence stamp. Skipped when there is nothing new, the same way `POST /consolidate` and `POST /consolidate/interim` are. |
...
| POST | `/speaker/forget` | admin | Remove a speaker's profile, erase their indexed-memory keys from every per-tier registry and SimHash map, and discard pending sessions. Registry-level erasure only — does not trigger retraining; a resident tier's weights are trimmed only by a later operator-invoked `POST /reconsolidate`. See [Forgetting a speaker](#forgetting-a-speaker-data-erasure). |
