...
Capacity tests show 95\% recall at 20 indexed keys, and incremental learning preserves old knowledge when new facts are added.
...
% Results teaser
In experiments on a single consumer GPU (8\,GB VRAM, QLoRA 4-bit), \paramem{} achieves 9/10 exact per-fact recall, scales to 95\% at 20 keys, and maintains 100\% recall across a 10-cycle consolidation loop with incremental key addition and episodic-to-semantic promotion.
...
A frozen base model (Qwen 2.5 3B, QLoRA 4-bit) hosts two named LoRA adapters that map to the episodic--semantic distinction in complementary learning systems theory~\citep{mcclelland1995complementary}:
...
Before arriving at indexed key retrieval, we explored two alternative approaches. Both failed in instructive ways (Table~\ref{tab:failed}).
...
(1)~\emph{Precise retrieval cues.} Indexed key recall achieves 90\% (9/10) versus 47\% for individual QA recall on the same adapter---keys are less ambiguous than natural-language questions.
...
All experiments use Qwen~2.5~3B with QLoRA 4-bit quantization on a single NVIDIA RTX~5070 (8\,GB VRAM).
Training uses the HuggingFace Trainer with prompt-token masking (labels set to $-100$) and the model's native chat template.
...
\begin{table}[t]
\centering
\caption{Capacity scaling at rank~8, 30~epochs. Hallucination tests use 5 untrained keys per condition.}
\label{tab:capacity}
\small
\begin{tabular}{@{}rrrr@{}}
\toprule
...
Scaling from 10 to 20 indexed pairs maintains 95\% recall with lower final training loss (1.069 vs.\ 1.263) at double the training data (Table~\ref{tab:capacity}).
...
Cycle time grows linearly with key count ($\sim$40\,s per additional key, dominated by reconstruction probing).
This is acceptable for personal-scale data (dozens to low hundreds of keys).
The linear trend suggests the approach will not hit a wall at moderate scale, but capacity beyond 20~keys has not been validated.
...
Consolidation is an offline batch process ($\sim$5\,min per cycle) suitable for running overnight---analogous to biological sleep consolidation~\citep{mcclelland1995complementary}.
Replay scheduling approaches such as FOREVER~\citep{liu2026forever} and MSSR~\citep{wu2026mssr} offer complementary strategies for optimizing which facts to replay and when; integrating these with our indexed key mechanism is a natural extension.
No external services are required beyond the base model weights.

\paragraph{Limitations.}
We are transparent about the current scope:
\begin{itemize}
    \item \textbf{Scale:} 17~keys tested in full consolidation, 20 in capacity tests. Real personal agents will need hundreds to thousands of facts.
    \item \textbf{Single synthetic user:} All experiments use a synthetic persona (``Alex''). Real users have more varied and complex fact structures.
    \item \textbf{Single model:} Qwen~2.5~3B only. The architecture is model-agnostic by design, but secondary model validation (Llama~3.2~3B) has not been completed for indexed keys.
    ...
    \item \textbf{Extraction noise:} The 3B model achieves $\sim$78\% extraction success with noisy predicates. The core contribution is indexed key retrieval, not extraction quality.
\end{itemize}
...
Testing 50--100+ indexed keys with dynamic rank allocation to accommodate growing memory.

\paragraph{Real user evaluation.}
A 30+ day longitudinal study with actual user sessions to validate beyond synthetic data.

\paragraph{Model-agnostic validation.}
Verifying indexed key retrieval on Llama~3.2~3B, Phi-3, and Gemma to establish generality.
...
The approach is demonstrated on edge hardware (8\,GB VRAM) with 100\% recall across 10 consolidation cycles, establishing that domain-scoped continual learning is feasible for personal LLM agents without external infrastructure.
